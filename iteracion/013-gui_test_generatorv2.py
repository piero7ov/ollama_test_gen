...
def build_prompt(apuntes_md: str, n_vf: int, n_short: int) -> str:
    ...
    # Construimos instrucciones SIN placeholders repetidos tipo "1. ..."
    # y SIN secciones que no aplican.
    sections = ["## Examen"]
    if n_vf > 0:
...
# ============================
#  Validación (para detectar el fallo que te pasó)
# ============================
def validate_output(md: str, n_vf: int, n_short: int) -> bool:
    ...
    # Respuesta corta: líneas "N. ..." pero NO "(V/F)"
    if n_short > 0:
    ...
    for k_str, content in ans_lines:
        ...
        # V/F: SOLO V o F
        if 1 <= k <= n_vf:
            if cl not in ("v", "f", "verdadero", "falso"):
                return False

        # Corta: NO puede ser V/F
        if n_vf < k <= total:
            if cl in ("v", "f", "verdadero", "falso"):
                return False
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.n_vf = tk.IntVar(value=0)
        self.n_short = tk.IntVar(value=10)
    ...
    def _build_ui(self):
        ...
        ttk.Label(row, textvariable=self.pdf_path, wraplength=700).pack(side="left", padx=10)
        ...
        ttk.Label(row2, textvariable=self.out_dir, wraplength=700).pack(side="left", padx=10)
    ...
    def _worker_generate(self, pdf_path: str, n_vf: int, n_short: int):
        try:
            ...
            # Validación + 1 reintento estricto
            if not validate_output(result, n_vf, n_short):
                self.msg_queue.put(("log", "⚠️ Salida con formato raro. Reintentando 1 vez (estricto + temp 0.0)..."))
                prompt2 = prompt + "\n\nREGLA FINAL: NO escribas respuestas dentro de '## Examen'. Las respuestas van SOLO en '## Respuestas'. Respeta numeración 1..N."
                result = ollama_generate_stream(
                    prompt2,
                    model=model,
                    host=host,
            ...
            # Si aún falla, guardamos igual para depurar
            if not validate_output(result, n_vf, n_short):
                self.msg_queue.put(("log", "❌ Ojo: sigue raro. Se guardó igual para que lo revises (debug)."))
